-- Critical Security Fixes - Phase 2: Data Access Restrictions (Simplified)
...
DROP POLICY IF EXISTS "Providers can manage their payment links v2" ON payment_links;

-- Create secure payment link policies
CREATE POLICY "Authenticated users can view their payment links v2" 
ON payment_links 
FOR SELECT 
USING (
...
CREATE POLICY "Providers can manage their payment links v3" 
ON payment_links 
FOR ALL 
...
DROP POLICY IF EXISTS "Authenticated users can view forum posts v2" ON forum_posts;
DROP POLICY IF EXISTS "Authenticated users can view forum replies v2" ON forum_replies;

-- Create authenticated-only forum policies
CREATE POLICY "Authenticated users can view forums v2" 
ON community_forums 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can view forum posts v3" 
ON forum_posts 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can view forum replies v3" 
ON forum_replies 
FOR SELECT 
USING (auth.uid() IS NOT NULL);
...
-- Create restricted provider access policies - Only authenticated users can see providers
...
DROP POLICY IF EXISTS "Authenticated users can view verified provider profiles v2" ON provider_profiles;

CREATE POLICY "Authenticated users can view verified provider profiles v3" 
...
DROP POLICY IF EXISTS "Users can create notifications for themselves v2" ON notifications;

CREATE POLICY "Users can create notifications for themselves v3" 
ON notifications 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

-- 6. UPDATE EXISTING POLICIES TO BE MORE RESTRICTIVE
DROP POLICY IF EXISTS "Patients can view their insurance claims" ON insurance_claims;
DROP POLICY IF EXISTS "Patients can view their insurance claims v2" ON insurance_claims;

CREATE POLICY "Patients can view their insurance claims v3" 
ON insurance_claims 
FOR SELECT 
...
-- 9. ADD RATE LIMITING FOR SECURITY-SENSITIVE OPERATIONS
