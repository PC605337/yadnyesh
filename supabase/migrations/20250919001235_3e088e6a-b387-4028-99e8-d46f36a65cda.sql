...
-- Insert initial admin role (you'll need to replace this UUID with your actual user ID)
-- This creates an admin role for the first user
...
-- Create trigger to auto-assign patient role to new users if no role specified
...
AS $$
BEGIN
    -- Insert default patient role if user doesn't have any roles
    INSERT INTO public.user_roles (user_id, role)
    SELECT NEW.id, 'patient'::app_role
    WHERE NOT EXISTS (
...
-- Create trigger on auth.users table (this will be created automatically)
-- Note: This needs to be done through Supabase dashboard as it's on the auth schema