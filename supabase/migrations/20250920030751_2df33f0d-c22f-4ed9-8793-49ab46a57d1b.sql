-- Critical Security Fixes - Phase 2: Data Access Restrictions (Final)
...
CREATE POLICY "Providers can manage their payment links final" 
ON payment_links 
FOR ALL 
...
CREATE POLICY "Authenticated users can view forums final" 
ON community_forums 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can view forum posts final" 
ON forum_posts 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can view forum replies final" 
ON forum_replies 
FOR SELECT 
USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "Public directory basic info final" 
ON healthcare_providers 
FOR SELECT 
USING (verification_status = 'verified');

CREATE POLICY "Authenticated users verified providers final" 
ON healthcare_providers 
FOR SELECT 
USING (
  auth.uid() IS NOT NULL AND 
...
CREATE POLICY "Authenticated users can view verified provider profiles final" 
...
CREATE POLICY "Users can create notifications for themselves final" 
ON notifications 
FOR INSERT 
...
CREATE POLICY "Patients can view their insurance claims final" 
ON insurance_claims 
FOR SELECT 
USING (auth.uid() = patient_id);

-- 10. Log successful completion with NULL user_id for system events
INSERT INTO security_audit_logs (user_id, event_type, event_details)
VALUES (
